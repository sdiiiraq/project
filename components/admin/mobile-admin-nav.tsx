...
export function MobileAdminNav() {
  ...
  return (
    <header className="flex h-14 items-center justify-between border-b bg-secondary px-4 text-secondary-foreground lg:hidden">
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        <SheetContent side="bottom" className="pb-8">
          ...
          <div className="mt-4 flex flex-col gap-1 border-t pt-4">
            <SheetClose asChild>
              <Link href="/dashboard" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-foreground hover:bg-muted">
                <ArrowRightLeft className="h-[18px] w-[18px]" /> رجوع لمولدتي
              </Link>
            </SheetClose>
            <form action={signOut}>
              <button type="submit" className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-destructive hover:bg-destructive/10">
